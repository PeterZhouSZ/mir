...
#include "StridedMemsetKernel.h"

extern "C" __global__ void stridedMemsetKernel(StridedMemsetKernelParams kernelParams)
{
    ...
    for (unsigned i = 0; i < kernelParams.dims[4]; i++) {
        for (unsigned z = 0; z < kernelParams.dims[2]; z++) {
            output[z * kernelParams.stridesOutput[1] + i * kernelParams.stridesOutput[3]] = kernelParams.value;
        }
    }
}
