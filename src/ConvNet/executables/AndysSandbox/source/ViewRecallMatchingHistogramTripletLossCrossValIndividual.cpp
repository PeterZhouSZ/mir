...
class ViewRecallMatchingHistogramTripletLossCrossValTrainer : public convnet::Trainer
{
    public:
        ...
        std::vector<Descriptors> computeAllDescriptors(HistogramDataSource &dataSource) {
            dataSource.restart();
        ...
        void runRanking(HistogramDataSource &dataSource, bool subjectBased, RankingResults &results) {
            ...
            std::vector<Descriptors> allDescriptors = computeAllDescriptors(dataSource);
        ...
        void runRankingTest(HistogramDataSource &dataSource, HistogramDataSource &testDataSource, RankingResults &results) {

            std::vector<Descriptors> allDescriptors = computeAllDescriptors(dataSource);
            std::vector<Descriptors> allTestDescriptors = computeAllDescriptors(testDataSource);
            
            unsigned numImages = 0;
            for (unsigned matchIdx = 0; matchIdx < dataSource.getHistograms().size(); matchIdx++)
				numImages = std::max(numImages, dataSource.getHistograms()[matchIdx].image);
			numImages++;
            
            for (unsigned queryIdx = 0; queryIdx < testDataSource.getHistograms().size(); queryIdx++) {
                std::cout << "Matching " << queryIdx << " of " << testDataSource.getHistograms().size() << std::endl;

				Matches matches;
				matches.resize(numImages);
				for (unsigned i = 0; i < numImages; i++) {
					matches[i].image = i;
					matches[i].score = 1e30f;
				}

                for (unsigned matchIdx = 0; matchIdx < dataSource.getHistograms().size(); matchIdx++) {
                    float difference = 0.0f;
                    for (unsigned i = 0; i < allTestDescriptors[queryIdx].recallDesc.size(); i++) {
                        float r = allTestDescriptors[queryIdx].recallDesc[i];
                        float v = allDescriptors[matchIdx].viewDesc[i];
                        difference += (r-v)*(r-v);
                    }

                    matches[dataSource.getHistograms()[matchIdx].image].score = std::min(matches[dataSource.getHistograms()[matchIdx].image].score, difference);
                }

				std::sort(matches.begin(), matches.end(), [](const Match &lhs, const Match &rhs)->bool{
                    return lhs.score < rhs.score;
                });

				results[SubjectImage{testDataSource.getHistograms()[queryIdx].subject, testDataSource.getHistograms()[queryIdx].image}] = std::move(matches);
            }
        }
...
#define ONE_VS_ALL
    ...
    struct SubjectResults {
		...
		SubjectResults() {
		    accuracyVsRank.resize(100, 0.0f);
		}
    };
    ...
    averageAccuracyVsRank.resize(100, 0.0f);
    ...
    auto updateResults = [&]() {
        ...
        main << "<h1>Average over " << averageAccuracyVsRankDenom << " subjects</h1>" << std::endl;
        ...
        main
            ...
            << "</html>" << std::endl;
#ifndef ONE_VS_ALL
        httpReport.putText(main, "matchingViewRecall_LeaveOneSubjectOut.html");
#else
        httpReport.putText(main, "matchingViewRecall_LeaveOneSubjectOut_OneAgainstAll.html");
#endif
    };
    ...
    for (unsigned fold = 0; fold < dataTrain.subjects.size(); fold++) {
    ...
    //        "d_view",
        ...
        HistogramData allData = dataTrain;

        HistogramDataSource testDataSource;
        testDataSource.takeSubjects(allData, {fold});
        
        std::cout << "Test data: " << testDataSource.getHistograms().size() << std::endl;

        HistogramDataSource trainingDataSource;
        trainingDataSource.takeAll(allData);

#ifndef ONE_VS_ALL
        bool subjectBased = true;
#else
        bool subjectBased = false;
#endif
        ...
        auto updateReport = [&](unsigned epoch, unsigned iter) {
            ...
            main
                ...
                << "</html>" << std::endl;
#ifndef ONE_VS_ALL
            httpReport.putText(main, "index_eye_LeaveOneSubjectOut.html");
#else
            httpReport.putText(main, "index_eye_LeaveOneSubjectOut_OneAgainstAll.html");
#endif
        };
...
#else
                trainer.runRankingTest(trainingDataSource, testDataSource, results);
#endif
                {
...
#else
                    trainer.runRankingTest(trainingDataSource, trainingDataSource, resultsTrain);
#endif
                    {
                        lastTrainROC.clear();
