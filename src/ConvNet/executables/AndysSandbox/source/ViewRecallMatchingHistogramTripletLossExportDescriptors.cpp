...
class ViewRecallMatchingHistogramTripletLossExpDescrTrainer : public convnet::Trainer
{
    public:
        ViewRecallMatchingHistogramTripletLossExpDescrTrainer(convnet::ConvNet &convnet, HistogramDataSource &dataSource) : Trainer(convnet, dataSource) { 
...
*/
        ...
        std::vector<Descriptors> computeDescriptors(HistogramDataSource &dataSource) {
            dataSource.restart();
...
void trainViewRecallMatchingHistogramTripletLossExportDescriptors(const eyeTracking::Dataset &dataset, HistogramData dataTrain, HistogramData dataTest, convnet::CuDnnWrapper &cudnnWrapper)
{
    std::mt19937 rng;
...
//        "d_view",
    ...
    trainingDataSource.takeAll(dataTrain);

    auto setDataSourceOutputs = [](HistogramDataSource &dataSource) {
        dataSource.viewOutput = 0;
        dataSource.recallOutput = 1;
        dataSource.opposingViewOutput = 2;
        dataSource.opposingRecallOutput = 3;
        
        dataSource.opposingSequenceFromSameSubject = true;
    };
    ...
    ViewRecallMatchingHistogramTripletLossExpDescrTrainer trainer(net, trainingDataSource);
    ...
    auto updateReport = [&](unsigned epoch, unsigned iter) {
        ...
        main
            ...
            << "</body>" << std::endl
            << "</html>" << std::endl;
        httpReport.putText(main, "index_eye2.html");
    };
    ...
    for (unsigned epoch = 0; epoch < 1000; epoch++) {
        ...
        if (epoch % 100 == 0) {
            ViewRecallMatchingHistogramTripletLossExpDescrTrainer::RankingResults resultsTrain;
            trainer.runRanking(trainingDataSource, true, resultsTrain);
            {
                lastTrainROC.clear();
    ...
    {
        std::vector<ViewRecallMatchingHistogramTripletLossExpDescrTrainer::Descriptors> descriptors = trainer.computeDescriptors(trainingDataSource);
        ...
        main << "<font size=\"2\"><pre>descriptors_view = [";
        for (unsigned i = 0; i < trainingDataSource.getHistograms().size(); i++) {
            if (i > 0)
                main << ", ";
            main << "[";
            for (unsigned j = 0; j < descriptors[i].viewDesc.size(); j++) {
                if (j > 0)
                    main << ", ";
                main << descriptors[i].viewDesc[j];
            }
            main << "]";
        ...
        main << "<font size=\"2\"><pre>descriptors_recall = [";
        for (unsigned i = 0; i < trainingDataSource.getHistograms().size(); i++) {
            if (i > 0)
                main << ", ";
            main << "[";
            for (unsigned j = 0; j < descriptors[i].recallDesc.size(); j++) {
                if (j > 0)
                    main << ", ";
                main << descriptors[i].recallDesc[j];
            }
            main << "]";
        ...
        main << "<font size=\"2\"><pre>descriptor_subject_image = [";
        for (unsigned i = 0; i < trainingDataSource.getHistograms().size(); i++) {
            if (i > 0)
                main << ", ";
            main << "(" << trainingDataSource.getHistograms()[i].subject << ',' << trainingDataSource.getHistograms()[i].image << ")";
        }
        main << "]</pre></font> " << std::endl;          

        main << "<font size=\"2\"><pre>subjects = [";
        for (unsigned s = 0; s < dataTrain.subjects.size(); s++) {
            if (s > 0)
                main << ", ";
            main << "'" << dataTrain.subjects[s].name << "'";
        }
        main << "]</pre></font> " << std::endl;  

        
        main << "<font size=\"2\"><pre>images = [";
        for (unsigned i = 0; i < dataTrain.images.size(); i++) {
            if (i > 0)
                main << ", ";
            main << "'" << dataTrain.images[i].filename << "'";
        }
        main << "]</pre></font> " << std::endl;          
        ...
        main
            << "</body>" << std::endl
            << "</html>" << std::endl;
        httpReport.putText(main, "matchingViewRecallDescriptors.html");
